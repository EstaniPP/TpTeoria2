...
public class Utilities {
	...
	public static BufferedImage parallelDecoder(ArrayList<Byte> encoded, int processors) {
		...
		*/
		...
		ExecutorService eImgGen = Executors.newFixedThreadPool(processors);
		...
		while(by < header.getWholeY()) {
			...
			while(bx < header.getWholeX()) {
				...
				eImgGen.execute(new ParallelImageGenerator(decoded, nuevaImagen, where, header, bn, bx, by));
				// moverme en x hasta la pos del ultimo columna del ultimo bloque
				bx += header.getX(bn);
				bn++;
			}
		...
		try {
			eImgGen.awaitTermination(Long.MAX_VALUE, TimeUnit.NANOSECONDS);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
